...
void open_lights(void)
{
    ...
    /*    LIGHT3    */
    GLfloat qaDiffuseLight3[] = { 0.8f, 0.8f, 0.8f, 1.0f };
...
void easter_egg_light()
{
    ...
    GLfloat qaDiffuseLight2[] = { 0.0f, 1.0f, 0.0f, 1.0f };
    GLfloat lightPosition2[] = { 0.0f, 0.0f, (K+2), 1.0f };
...
void ifgameover()
{
    for (auto tempiter = cubelist.begin(); tempiter != cubelist.end(); tempiter++)
    {
        if (tempiter->retY() == K)
            game_over = true;
    }
...
void Create_Shape()
{
    ...
    std::list<int> l; // list that contains the already existing neighbor cubes
    std::list<int>::iterator it; // list that contains the new candidates neighbor cubes
    ...
    if (temp_color == YELLOW) {
        ...
        for (i = 0; i < 27; i++) {
            if (black[c][i] != -1) {
                ...
                for (auto iter2 = cubelist.begin(); iter2 != cubelist.end(); iter2++)
                {
        ...
        int blue_c = 0;
        std::list<Cube> templist;
        std::list<Cube>::iterator tempiter;
        ...
        int red_c = 0;
        std::list<Cube> templist2;
        std::list<Cube>::iterator tempiter2;
...
{
    char* c;
    glRasterPos3f(x_local, y_local, z_local);
    for (c = str; *c != '\0'; c++)
    {
...
void RenderSceneCallback::processSpecialKeys(int key, int, int)
{
    if (game_over == false)
    {
        switch (key)
...
void RenderSceneCallback::processNormalKeys(unsigned char key, int, int)
{
    if (key == 27)
    {
        exit(0);
...
void RenderSceneCallback::renderScene()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if (camera_count == 0)
    {
        ...
        if (game_over == false)
        {
            if (((limits_d == DOWN_LEVEL) && (syn_y == -K + 1)) ||
                ...
                {
                if (start == 1)
                { // if there is at least one volume
                    puttocosmos();
                    ifgameover();
                    reached = false;
                    score++;
    ...
    if (paused == true)
    {
        if (camera_count == 0)
        {
            ...
            draw_text(x + 3.0f, y - 2.8f, z - 5.0f, (char*)"GAME PAUSED");
            draw_text(x + 2.5f, y - 3.25f, z - 5.0f, (char*)"PRESS");
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(x + 3.5f, y - 3.63f, z - 5.0f, (char*)"S");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(x + 4.0f, y - 3.8f, z - 5.0f, (char*)"TO CONTINUE");
            glPopMatrix();

            glPushMatrix();
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(x + 7.0f, y - 2.5f, z - 5.0f, (char*)"SCORE : ");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(x + 9.0f, y - 2.9f, z - 5.0f, s1);
            glPopMatrix();
        }
        else if (camera_count == 1)
        {
            glPushMatrix();
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(-0.5f, camera2_y - K, 0.0f, (char*)"GAME PAUSED");
            draw_text(-1.0f, camera2_y - K, 0.5f, (char*)"PRESS");
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(0.0f, camera2_y - K, 0.5f, (char*)"S");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(0.5f, camera2_y - K, 0.5f, (char*)"TO CONTINUE");
            glPopMatrix();

            glPushMatrix();
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(1.0f, camera2_y - 3.0f, -0.2f, (char*)"SCORE : ");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(1.4f, camera2_y - 3.0f, -0.2f, s1);
            glPopMatrix();
        }
        else if (camera_count == 2)
        {
            glPushMatrix();
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(-0.5f, 0.5f, camera3_z - 5.0f, (char*)"GAME PAUSED");
            draw_text(-0.8f, -0.2f, camera3_z - 5.0f, (char*)"PRESS");
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(-0.2f, -0.2f, camera3_z - 5.0f, (char*)"S");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(0.0f, -0.2f, camera3_z - 5.0f, (char*)"TO CONTINUE");
            glPopMatrix();

            glPushMatrix();
            glColor3f(1.0f, 0.11f, 0.68f);
            draw_text(0.65f, 0.65f, camera3_z - 2.0f, (char*)"SCORE : ");
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(0.92f, 0.65f, camera3_z - 2.0f, s1);
            glPopMatrix();
        }
    }
    ...
    if (camera_count == 0)
    {
        ...
        if (game_over == true)
        {
            glPushMatrix();
            glColor3f(0.0f, 1.0f, 0.0f);
            draw_text(x + 3.0f, y - 4.5f, z - 5.0f, (char*)"GAME OVER");
            glPopMatrix();
        }
    }
    else if (camera_count == 1)
    ...
    if (easter_egg == true)
    {
        ...
        draw_text(x + 0.5f, y - 2.0f, z - 3.0f, (char*)"YOU ARE EXCELENT!!  YOU MANAGED TO MAKE 500 POINTS!!!!");
        glPopMatrix();
    }
...
void RenderSceneCallback::changeSize(int w, int h)
{
    if (h == 0) // Prevent a divide by zero, when window is too short
        h = 1;
    float ratio = static_cast<float>(w) * 1.0f / static_cast<float>(h);
...
void RenderSceneCallback::render()
{
        ...
        glPopMatrix();
            ...
            if(camera_count==0){
                ...
                glPushMatrix();
                    ...
                    draw_text(0.65f,0.65f ,camera3_z-2.0f,(char*)"SCORE : ");
                    glColor3f(0.0f,1.0f,0.0f);
                    draw_text(0.92f,0.65f, camera3_z-2,s1);
                glPopMatrix();
            }
